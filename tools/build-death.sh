...
usage()
{
    ...
    echo -e "  build-death.sh [options] device"
    echo -e ""
    ...
    echo -e "    ./build-death.sh -p -o3 -j18 hammerhead"
    echo -e ""
    exit 1
}

# colors
. ./vendor/reaper/tools/colors

if [ ! -d ".repo" ]; then
    echo -e ${red}"No .repo directory found.  Is this an Android build tree?"${txtrst}
    exit 1
fi
if [ ! -d "vendor/reaper" ]; then
    echo -e ${red}"No vendor/reaper directory found.  Is this an ReaperOS build tree?"${txtrst}
    exit 1
fi
...
# get current version
eval $(grep "^PRODUCT_VERSION_" vendor/reaper/config/common.mk | sed 's/ *//g')
VERSION="$PRODUCT_VERSION_MAINTENANCE.$REAPER_VERSION_MAJOR."

echo -e ${bldgrn}"Building Reaper $VERSION"${txtrst}
...
if [ "$opt_clean" -eq 0 ]; then
    ...
    echo -e ""
    echo -e ${red}"Good work assholes #StayDead"${txtrst}
    echo -e ""
...
if [ "$opt_log" -ne 0 ]; then
    ...
    make -j"$opt_jobs" reaper 2>&1 | tee build-logs/reaper_$device-$(date "+%Y%m%d").txt
else
    # build normally
    make -j"$opt_jobs" death
